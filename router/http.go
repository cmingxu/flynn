...
func (h *httpSyncHandler) Set(data *router.Route) error {
	...
	if service == nil {
		...
		service = newService(r.Service, sc, h.l.wm)
	...
	go h.l.wm.Send(&router.Event{Event: router.EventTypeSet, ID: r.Domain, Route: r.ToRoute()})
	return nil
}
...
func (h *httpSyncHandler) Remove(id string) error {
	...
	go h.l.wm.Send(&router.Event{Event: router.EventTypeRemove, ID: id, Route: r.ToRoute()})
	return nil
}
...
func newService(name string, sc *cache.ServiceCache, wm *WatchManager) *service {
	s := &service{
		name: name,
		sc:   sc,
		wm:   wm,
...
func (s *service) watchBackends() {
	events := make(chan *discoverd.Event)
	s.stream = s.sc.Watch(events, true)
	go func() {
		for event := range events {
			if event.Kind == discoverd.EventKindDown {
				go s.sendDrainEvent(&router.Backend{
					Service: s.name,
...
func (s *service) sendDrainEvent(backend *router.Backend) {
	s.cond.L.Lock()
